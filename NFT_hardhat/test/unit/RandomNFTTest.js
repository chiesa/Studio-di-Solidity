...
!developmentChains.includes(network.name)
    ...
    : describe("Random IPFS NFT Unit Tests", function () {
          let randomIpfsNft, deployer, vrfCoordinatorV2
          ...
          describe("requestNft", () => {
              ...
              it("emits an event and kicks off a random word request", async function () {
                  ...
                  await expect(randomIpfsNft.mintNFT({ value: fee })).to.emit(
                      randomIpfsNft,
                      "NFTRequest"
                  )
              })
            ...
            it("mints NFT after random number is returned", async function () {
                ...
                await new Promise( async (res,rej) => {
                    try {
                        ...
                        const requestNftResponse = await randomIpfsNft.mintNFT({ value: fee })
                        console.log("1")
          ...
          describe("getType", () => {
              ...
              it("should revert if moddedRng > 99", async function () {
                  await expect(randomIpfsNft.getType(100)).to.be.revertedWith(
                      "RandomIPFS__InvalidRandomNumber()"
                  )
              })
          })
      })